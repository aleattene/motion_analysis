...
def get_elbow_wrist_pinky_angle(source):
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        ...
        while cap.isOpened():
            ...
            try:
                ...
                pinky = [landmarks[mp_pose.PoseLandmark.RIGHT_PINKY.value].x,
                ...
                cv2.circle(image, tuple(np.multiply(wrist, [image.shape[1], image.shape[0]]).astype(int)), 5,
                           (0, 0, 255), -1)
                cv2.circle(image, tuple(np.multiply(pinky, [image.shape[1], image.shape[0]]).astype(int)), 5,
                           (0, 0, 255), -1)

                # Calculate the angle
                angle_revealed_one = calculate_angle_degrees(elbow, wrist, pinky)
                if angle_offset_one is None:
                    angle_offset_one = 105 - angle_revealed_one
        ...
        result_one = f"Gomito-Polso-Mano (gradi): {str(int(angle_one))}"
