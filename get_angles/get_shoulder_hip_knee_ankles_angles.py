...
def get_shoulder_hip_knee_ankle_angles(source):
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        ...
        while cap.isOpened():
            ...
            try:
                ...
                cv2.line(image, tuple(np.multiply(hip, [image.shape[1], image.shape[0]]).astype(int)),
                         ...
                         (0, 255, 0), 2)
                cv2.line(image, tuple(np.multiply(knee, [image.shape[1], image.shape[0]]).astype(int)),
                         tuple(np.multiply(ankle, [image.shape[1], image.shape[0]]).astype(int)),
                         (0, 255, 0), 2)
                ...
                # Calculate the angle
                angle_revealed_one = calculate_angle_degrees(shoulder, hip, knee)
                angle_revealed_two = calculate_angle_degrees(hip, knee, ankle)
            ...
            if angle_one < 90:
                rect_color_one = GREEN_COLOR
            else:
                rect_color_one = LIGHT_BLUE_COLOR

            if angle_two < 170:
                rect_color_two = RED_COLOR
            else:
                rect_color_two = LIGHT_BLUE_COLOR

            # Setup status box with angle
            cv2.rectangle(image, (0, 550), (170, 450), rect_color_one, -1)
            cv2.putText(image, f"{str(int(angle_one))}",
                        (20, 520),
                        cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 255, 255), 2, cv2.LINE_AA)

            cv2.rectangle(image, (0, 670), (170, 570), rect_color_two, -1)
        ...
        result_two = f"Anca-Ginocchio-Caviglia (gradi): {str(int(angle_two))}"
