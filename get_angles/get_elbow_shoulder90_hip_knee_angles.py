...
def get_elbow_shoulder90_hip_knee_angles(source):
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        ...
        while cap.isOpened():
            ...
            try:
                ...
                cv2.line(image, tuple(np.multiply(elbow, [image.shape[1], image.shape[0]]).astype(int)),
                ...
                if angle_offset_one is None:
                    angle_offset_one = 120 - angle_revealed_one
                if angle_offset_two is None:
                    angle_offset_two = 120 - angle_revealed_two
        ...
        result_one = f"Gomito-Spalla-Anca (gradi): {str(int(angle_one))}"
        result_two = f"Spalla-Anca-Ginocchio (gradi): {str(int(angle_two))}"
