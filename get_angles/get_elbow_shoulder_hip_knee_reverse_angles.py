...
import numpy as np
import os
from motion_analysis.utils.angles import calculate_angle_degrees, calculate_angle_reverse_degrees


# Function to analyze the source (stream or video)
def get_elbow_shoulder_hip_knee_reverse_angles(source):
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        ...
        while cap.isOpened():
            ...
            try:
                ...
                angle_revealed_two = calculate_angle_reverse_degrees(shoulder, hip, knee)
                print(f"Angle one: {angle_revealed_one}, Angle two: {angle_revealed_two}")

                if angle_offset_one is None:
                    angle_offset_one = 140 - angle_revealed_one
                if angle_offset_two is None:
                    angle_offset_two = 200 - angle_revealed_two
            ...
            if angle_one > 170:
                ...
                rect_color_one = LIGHT_BLUE_COLOR  # Original color (light blue)

            if angle_two < 180:
                rect_color_two = RED_COLOR  # Red
            elif 180 <= angle_two <= 200:
                rect_color_two = LIGHT_BLUE_COLOR  # Red
            else:
                rect_color_two = GREEN_COLOR  # Original color (light blue)
